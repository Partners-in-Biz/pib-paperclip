...
export function createFakeDb(store: Store, options: FakeDbOptions) {
  ...
  function insert(sql: string, params: unknown[]): { rowCount: number } {
    ...
    if (existing) {
      ...
      if (m[6]) {
        // Conditional upsert: `<table>.col <= EXCLUDED.col` (and <, >=, >), compared as times.
        const cond = /^(?:\S+\.)?(\w+)\s*(<=|>=|<|>)\s*EXCLUDED\.(\w+)$/i.exec(m[6].trim());
        if (!cond) throw new Error(`fake db: unsupported upsert condition "${m[6]}"`);
        const left = timeOf(existing[cond[1]!]);
        const right = timeOf(row[cond[3]!]);
        const op = cond[2]!;
        const ok = op === "<=" ? left <= right : op === ">=" ? left >= right : op === "<" ? left < right : left > right;
        if (!ok) return { rowCount: 0 };
      }
